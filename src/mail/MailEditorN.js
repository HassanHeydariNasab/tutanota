...
export function newMailEditorAsResponse(args: ResponseMailParameters, mailboxDetails: ?MailboxDetail, blockExternalContent: boolean, inlineImages: ?Promise<InlineImages>): Promise<Dialog> {
	const init = (mailbox) => {
		return defaultSendMailModel(mailbox).initAsResponse(args).then(model => {
			return createMailEditorDialog(model, blockExternalContent, inlineImages || undefined)
		})
	}
...
export function writeSupportMail(subject: string = "", mailboxDetails?: MailboxDetail) {
	...
	if (logins.getUserController().isPremiumAccount()) {
		const show = mailbox => {
			...
			newMailEditorFromTemplate(mailbox, recipients, subject, "<br/>" + getSupportMailSignature()).then(dialog => dialog.show())
		}
