...
export class SendMailModel {
	...
	send(
		...
		blockingWaitHandler: (TranslationKey | lazy<string>, Promise<any>) => Promise<any> = (_, p) => p,
		...
		return subjectConfirm
			...
			// Resolve recipient contacts and send if everything looks ok
			.then(confirmed => {
				if (!confirmed) {
					return false
				} else {
					let recipients
					const sendPromise = this._waitForResolvedRecipients()
					                        .then(r => { recipients = r })
					                        .then(() => this.saveDraft(true, mailMethod))
