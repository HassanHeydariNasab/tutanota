...
o.spec("SendMailModel", () => {
	...
	o.spec("Sending", function () {
		...
		o("existing and new contacts", async function () {
			...
			const numContactsToSetup = model.allRecipients().filter(r => r.contact == null || r.contact._id
				== null).length

			const numContactsToUpdate = model.allRecipients().filter(r => isExternal(r) && r.contact
				&& r.contact.presharedPassword
				!== model.getPassword(r.mailAddress)).length
